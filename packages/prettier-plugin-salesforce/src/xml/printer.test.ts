...
describe("metadata XML printer", () => {
  ...
  it("keeps metadata transforms enabled on metadata suffix paths", async () => {
    const source =
      "<CustomObject><label>Widget</label><sharingModel>ReadWrite</sharingModel></CustomObject>";
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/objects/Widget__c/Widget__c.object-meta.xml",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on metadata suffix tmp paths", async () => {
    const source =
      "<PermissionSet><label>Sample App</label><hasActivationRequired>false</hasActivationRequired></PermissionSet>";
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/permissionsets/SampleApp.permissionset-meta.xml.tmp",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on metadata suffix windows paths", async () => {
    ...
    const formatted = await prettier.format(source, {
      filepath:
        "C:\\repo\\force-app\\main\\default\\objects\\Widget__c\\Widget__c.object-meta.xml",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on labels extension paths", async () => {
    const source =
      "<CustomLabels><labels><fullName>Welcome</fullName></labels></CustomLabels>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on Salesforce manifest package.xml paths", async () => {
    const source =
      "<Package><types><name>ApexClass</name><members>*</members></types><version>60.0</version></Package>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on Salesforce unpackaged package.xml paths", async () => {
    const source =
      "<Package><types><name>CustomObject</name><members>Widget__c</members></types><version>60.0</version></Package>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on Salesforce destructiveChanges.xml paths", async () => {
    const source =
      "<Package><types><name>ApexClass</name><members>ObsoleteClass</members></types><version>60.0</version></Package>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on Salesforce src/package.xml paths", async () => {
    const source =
      "<Package><types><name>CustomObject</name><members>*</members></types><version>60.0</version></Package>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on Salesforce manifest delta.xml package roots", async () => {
    const source =
      "<Package><types><name>ApexClass</name><members>Widget</members></types><version>60.0</version></Package>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on Salesforce lib profile package roots", async () => {
    const source =
      "<Package><types><name>Profile</name><members>Admin</members></types><version>60.0</version></Package>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on exported Salesforce profile xml outside source-tree paths", async () => {
    const source =
      "<Profile><custom>false</custom><userLicense>Salesforce</userLicense></Profile>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on ruleset xml outside source-tree paths", async () => {
    const source =
      '<ruleset name="Salesforce"><description>Rules</description><rule ref="category/apex/bestpractices.xml/AvoidGlobalModifier"/></ruleset>';
    const formatted = await prettier.format(source, {
  ...
  it("keeps labels entry order by default", async () => {
    ...
    expect(formatted.indexOf("<fullName>Zeta</fullName>")).toBeLessThan(
      formatted.indexOf("<fullName>Alpha</fullName>"),
    );
  });
  ...
  it("sorts labels entries by fullName when option is enabled", async () => {
    ...
    expect(formatted.indexOf("<fullName>Alpha</fullName>")).toBeLessThan(
  ...
  it("keeps leading comments attached to labels when sorting by fullName", async () => {
    ...
    expect(formatted.indexOf("<!-- Alpha help text -->")).toBeLessThan(
      formatted.indexOf("<fullName>Alpha</fullName>"),
    );
    expect(formatted.indexOf("<!-- Zeta help text -->")).toBeLessThan(
      formatted.indexOf("<fullName>Zeta</fullName>"),
    );
    expect(formatted.indexOf("<!-- Alpha help text -->")).toBeLessThan(
      formatted.indexOf("<!-- Zeta help text -->"),
    );
  });
  ...
  it("sorts labels entries for labels-meta.xml sidecar paths", async () => {
    ...
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/labels/CustomLabels.labels-meta.xml",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
      salesforceSortLabelsByFullName: true,
    });
  ...
  it("sorts labels entries for labels-meta.xml.tmp sidecar paths", async () => {
    ...
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/labels/CustomLabels.labels-meta.xml.tmp",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
      salesforceSortLabelsByFullName: true,
    });
  ...
  it("sorts mixed-case label fullName values alphabetically by fullName", async () => {
    ...
    expect(firstPass.indexOf("<fullName>Beta</fullName>")).toBeLessThan(
      firstPass.indexOf("<fullName>zeta</fullName>"),
    );
  ...
  it("sorts accented and mixed-case label fullName values deterministically", async () => {
    ...
    expect(firstPass.indexOf("<fullName>alpha</fullName>")).toBeLessThan(
      firstPass.indexOf("<fullName>eclair</fullName>"),
    );
    expect(firstPass.indexOf("<fullName>eclair</fullName>")).toBeLessThan(
      firstPass.indexOf("<fullName>Éclair</fullName>"),
    );
    expect(firstPass.indexOf("<fullName>Éclair</fullName>")).toBeLessThan(
      firstPass.indexOf("<fullName>Zebra</fullName>"),
    );
    expect(secondPass).toBe(firstPass);
  });
  ...
  it("sorts labels entries when labels/fullName tags carry attributes", async () => {
    ...
    expect(
      formatted.indexOf('<fullName xml:lang="en">Alpha</fullName>'),
    ).toBeLessThan(
  ...
  it("keeps label order when any labels block has no fullName", async () => {
    ...
    expect(formatted.indexOf("<fullName>Zeta</fullName>")).toBeLessThan(
      formatted.indexOf("<value>No name</value>"),
    );
  });
  ...
  it("keeps missing-fullName label order stable across repeated passes", async () => {
    ...
    expect(firstPass.indexOf("<fullName>Zeta</fullName>")).toBeLessThan(
      firstPass.indexOf("<value>No name 1</value>"),
    );
    expect(firstPass.indexOf("<value>No name 1</value>")).toBeLessThan(
      firstPass.indexOf("<fullName>Alpha</fullName>"),
    );
    expect(firstPass.indexOf("<fullName>Alpha</fullName>")).toBeLessThan(
      firstPass.indexOf("<value>No name 2</value>"),
    );
    expect(secondPass).toBe(firstPass);
  });
  ...
  it("keeps metadata transforms enabled on layouts directory legacy file forms", async () => {
    const source =
      "<Layout><layoutSections><label>Information</label></layoutSections></Layout>";
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/layouts/Widget__c-Widget Layout.layout",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on profiles directory legacy file forms", async () => {
    const source =
      "<Profile><custom>true</custom><userLicense>Salesforce</userLicense></Profile>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on permissionsets directory legacy file forms", async () => {
    const source =
      "<PermissionSet><label>App</label><hasActivationRequired>false</hasActivationRequired></PermissionSet>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on flows directory legacy file forms", async () => {
    const source =
      "<Flow><label>Widget Status</label><status>Active</status><processType>Flow</processType></Flow>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on translations directory legacy file forms", async () => {
    const source =
      "<Translations><customLabels><label>Bonjour</label><name>Greeting</name></customLabels></Translations>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on tabs directory legacy file forms", async () => {
    const source =
      "<CustomTab><description>Widget workbench</description><label>Widget</label><motif>Custom1: Heart</motif></CustomTab>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on flexipages directory legacy file forms", async () => {
    ...
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/flexipages/Widget_Record_Page.flexipage",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on applications directory legacy file forms", async () => {
    const source =
      "<CustomApplication><description>Console app</description><label>Console</label></CustomApplication>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on quickActions directory legacy file forms", async () => {
    const source =
      "<QuickAction><label>Assign</label><type>Update</type></QuickAction>";
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/quickActions/Widget__c.Assign.quickAction",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on workflows directory legacy file forms", async () => {
    const source =
      "<Workflow><alerts><fullName>Notify_Owner</fullName></alerts></Workflow>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on sharingRules directory legacy file forms", async () => {
    ...
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/sharingRules/Widget__c.sharingRules",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on standardValueSets directory legacy file forms", async () => {
    ...
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/standardValueSets/CasePriority.standardValueSet",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on globalValueSets directory legacy file forms", async () => {
    ...
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/globalValueSets/WidgetStatus.globalValueSet",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on pathAssistants directory legacy file forms", async () => {
    ...
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/pathAssistants/Widget__c-Widget.pathAssistant",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on remoteSiteSettings directory legacy file forms", async () => {
    const source =
      "<RemoteSiteSetting><disableProtocolSecurity>false</disableProtocolSecurity><isActive>true</isActive></RemoteSiteSetting>";
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/remoteSiteSettings/ERP_Endpoint.remoteSite",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on assignmentRules directory legacy file forms", async () => {
    ...
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/assignmentRules/Lead.assignmentRules",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on escalationRules directory legacy file forms", async () => {
    ...
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/escalationRules/Case.escalationRules",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on matchingRules directory legacy file forms", async () => {
    ...
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/matchingRules/Contact.matchingRule",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on duplicateRules directory legacy file forms", async () => {
    ...
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/duplicateRules/Contact_Dupe.duplicateRule",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on emailServices directory legacy file forms", async () => {
    ...
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/emailServices/CaseInbound.emailService",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on autoResponseRules directory legacy file forms", async () => {
    ...
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/autoResponseRules/Lead.autoResponseRules",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on namedCredentials directory legacy file forms", async () => {
    const source =
      "<NamedCredential><label>ERP</label><endpoint>https://erp.example.com</endpoint></NamedCredential>";
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/namedCredentials/ERP.namedCredential",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on externalCredentials directory legacy file forms", async () => {
    ...
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/externalCredentials/ERP.externalCredential",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on dashboards directory legacy file forms", async () => {
    const source =
      "<Dashboard><title>Sales KPI</title><runningUser>sales@example.com</runningUser></Dashboard>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on reports directory legacy file forms", async () => {
    ...
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/reports/Sales/Quarterly_Pipeline.report",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on groups directory legacy file forms", async () => {
    const source =
      "<Group><doesIncludeBosses>false</doesIncludeBosses><name>Support</name></Group>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on queues directory legacy file forms", async () => {
    const source =
      "<Queue><doesSendEmailToMembers>true</doesSendEmailToMembers><name>Support</name></Queue>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on connectedApps directory legacy file forms", async () => {
    const source =
      "<ConnectedApp><label>Widget Portal</label><contactEmail>owner@example.com</contactEmail></ConnectedApp>";
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/connectedApps/WidgetPortal.connectedApp",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on roles directory legacy file forms", async () => {
    const source =
      "<Role><name>Support Manager</name><opportunityAccessForAccountOwner>Read</opportunityAccessForAccountOwner></Role>";
    const formatted = await prettier.format(source, {
  ...
  it("keeps metadata transforms enabled on approvalProcesses directory legacy file forms", async () => {
    const source =
      "<ApprovalProcess><active>true</active><label>Widget Approval</label></ApprovalProcess>";
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/approvalProcesses/Widget__c.Widget_Approval.approvalProcess",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata transforms enabled on homePageLayouts directory legacy file forms", async () => {
    const source =
      "<HomePageLayout><narrowComponents><type>TaskList</type></narrowComponents></HomePageLayout>";
    const formatted = await prettier.format(source, {
      filepath:
        "/tmp/force-app/main/default/homePageLayouts/Home.homePageLayout",
      parser: "salesforce-metadata-xml",
      plugins: [plugin],
    });
  ...
  it("keeps metadata family paths format-safe on newly added file forms", async () => {
    const matrix = [
      {
        filepath:
          "/tmp/force-app/main/default/businessProcesses/Account.Standard.businessProcess",
        source:
          "<BusinessProcess><isActive>true</isActive><values><fullName>New</fullName></values></BusinessProcess>",
        expectedRootTag: "BusinessProcess",
      },
      {
        filepath:
          "/tmp/force-app/main/default/recordTypes/Account.B2B.recordType",
        source:
          "<RecordType><active>true</active><label>B2B</label></RecordType>",
        expectedRootTag: "RecordType",
      },
      {
        filepath:
          "/tmp/force-app/main/default/validationRules/Account.Require_Website.validationRule",
        source:
          "<ValidationRule><active>true</active><errorConditionFormula>ISBLANK(Website)</errorConditionFormula></ValidationRule>",
        expectedRootTag: "ValidationRule",
      },
      {
        filepath:
          "/tmp/force-app/main/default/webLinks/Account.OpenPortal.webLink",
        source:
          "<WebLink><availability>online</availability><masterLabel>Open Portal</masterLabel></WebLink>",
        expectedRootTag: "WebLink",
      },
      {
        filepath:
          "/tmp/force-app/main/default/listViews/Account.Recent_Active.listView",
        source:
          "<ListView><label>Recent Active</label><sharedTo><allInternalUsers>true</allInternalUsers></sharedTo></ListView>",
        expectedRootTag: "ListView",
      },
      {
        filepath:
          "/tmp/force-app/main/default/milestoneTypes/SLA_Response.milestoneType",
        source:
          "<MilestoneType><description>Response SLA</description><recurrenceType>noRecurrence</recurrenceType></MilestoneType>",
        expectedRootTag: "MilestoneType",
      },
      {
        filepath:
          "/tmp/force-app/main/default/milestones/Case_First_Response.milestone",
        source:
          "<Milestone><description>First response</description><startDateField>Name</startDateField></Milestone>",
        expectedRootTag: "Milestone",
      },
      {
        filepath: "/tmp/force-app/main/default/sites/Support.site",
        source:
          "<CustomSite><active>true</active><label>Support</label></CustomSite>",
        expectedRootTag: "CustomSite",
      },
      {
        filepath: "/tmp/force-app/main/default/networks/Partner.network",
        source:
          "<Network><description>Partner community</description><status>Live</status></Network>",
        expectedRootTag: "Network",
      },
      {
        filepath:
          "/tmp/force-app/main/default/sharingReasons/Deal_Team.sharingReason",
        source:
          "<SharingReason><label>Deal Team</label><name>Deal_Team</name></SharingReason>",
        expectedRootTag: "SharingReason",
      },
      {
        filepath:
          "/tmp/force-app/main/default/samlSsoConfigs/CorpSso.samlSsoConfig",
        source:
          "<SamlSsoConfig><issuer>urn:corp</issuer><version>2.0</version></SamlSsoConfig>",
        expectedRootTag: "SamlSsoConfig",
      },
      {
        filepath:
          "/tmp/force-app/main/default/corsWhitelistOrigins/Portal.corsWhitelistOrigin",
        source:
          "<CorsWhitelistOrigin><isActive>true</isActive><urlPattern>https://portal.example.com</urlPattern></CorsWhitelistOrigin>",
        expectedRootTag: "CorsWhitelistOrigin",
      },
      {
        filepath:
          "/tmp/force-app/main/default/customMetadata/Finance_Threshold.Finance_Setting.md",
        source:
          "<CustomMetadata><label>Finance Threshold</label><protected>false</protected></CustomMetadata>",
        expectedRootTag: "CustomMetadata",
      },
      {
        filepath: "/tmp/force-app/main/default/objects/Invoice__c.object",
        source:
          "<CustomObject><label>Invoice</label><pluralLabel>Invoices</pluralLabel></CustomObject>",
  ...
  it("passes through app extension paths outside applications directories", async () => {
    const source =
      '<aura:application><aura:text value="Hi"/></aura:application>\n\n';
    const formatted = await prettier.format(source, {
  ...
  it("preserves comments between sibling metadata elements", () => {
    ...
    expect(extractElementOrder(formatted)).toEqual([
      "CustomObject",
      "label",
      "sharingModel",
    ]);
  ...
  it("keeps permissionset sibling block order with declaration and comments", () => {
    const source = readFixture(
      "permissionset/SiblingRisk.permissionset-meta.xml",
    );
    const formatted = formatXmlConservative(source);

    expect(extractElementOrder(formatted)).toEqual(extractElementOrder(source));
    expect(formatted.indexOf("<applicationVisibilities>")).toBeLessThan(
      formatted.indexOf("<classAccesses>"),
    );
    expect(formatted.indexOf("<classAccesses>")).toBeLessThan(
      formatted.indexOf("<fieldPermissions>"),
    );
    expect(
      formatted.indexOf("<!-- app visibility before class access -->"),
    ).toBeGreaterThan(formatted.indexOf("<applicationVisibilities>"));
    expect(
      formatted.indexOf("<!-- class access before field perms -->"),
    ).toBeGreaterThan(formatted.indexOf("<classAccesses>"));
    expect(
      formatted.startsWith('<?xml version="1.0" encoding="UTF-8"?>\n'),
    ).toBe(true);
  });
  ...
  it("keeps profile metadata sequence stable and retains declaration/comment", () => {
    ...
    expect(extractElementOrder(formatted)).toEqual(
      extractElementOrder(withEnvelope),
    );
  });
  ...
  it("keeps profile sibling block order with declaration and comments", () => {
    ...
    expect(formatted.indexOf("<fieldPermissions>")).toBeLessThan(
      formatted.indexOf("<tabVisibilities>"),
    );
    expect(
      formatted.indexOf("<!-- apps before field perms -->"),
    ).toBeGreaterThan(formatted.indexOf("<applicationVisibilities>"));
    expect(
      formatted.indexOf("<!-- field perms before tabs -->"),
    ).toBeGreaterThan(formatted.indexOf("<fieldPermissions>"));
    expect(
      formatted.startsWith('<?xml version="1.0" encoding="UTF-8"?>\n'),
    ).toBe(true);
  });
  ...
  it("keeps layout metadata sequence stable and retains declaration/comment", () => {
    const source = readFixture(
      "layout/Widget__c-Widget Layout.layout-meta.xml",
    );
    const withEnvelope = [
      '<?xml version="1.0" encoding="UTF-8"?>',
      "<!-- layout guard -->",
  ...
  it("keeps layout sibling block order with declaration and comments", () => {
    ...
    expect(extractElementOrder(formatted)).toEqual(extractElementOrder(source));
    expect(formatted.indexOf("<layoutSections>")).toBeLessThan(
      formatted.indexOf("<relatedLists>"),
    );
    expect(formatted.indexOf("<relatedLists>")).toBeLessThan(
      formatted.indexOf("<showEmailCheckbox>"),
    );
    expect(formatted.indexOf("<!-- sections before lists -->")).toBeGreaterThan(
      formatted.indexOf("<layoutSections>"),
    );
    expect(formatted.indexOf("<!-- lists before settings -->")).toBeGreaterThan(
      formatted.indexOf("<relatedLists>"),
    );
    expect(
      formatted.startsWith('<?xml version="1.0" encoding="UTF-8"?>\n'),
  ...
  it("keeps flow metadata sequence stable and retains declaration/comment", () => {
    const source = readFixture(
      "flow/Widget_Status.with-comments.flow-meta.xml",
    );
    const formatted = formatXmlConservative(source);

    expect(formatted.includes('<?xml version="1.0" encoding="UTF-8"?>')).toBe(
      true,
    );
    expect(formatted.includes("<!-- keep process shape -->")).toBe(true);
    expect(
      formatted.includes("<!-- status remains after process type -->"),
    ).toBe(true);
    expect(extractElementOrder(formatted)).toEqual(extractElementOrder(source));
  });
  ...
  it("keeps mixed sibling block shape stable across metadata families", () => {
    for (const variant of metadataMixedShapeVariants) {
      ...
      for (let index = 0; index < variant.anchors.length - 1; index += 1) {
        expect(formatted.indexOf(variant.anchors[index])).toBeLessThan(
          formatted.indexOf(variant.anchors[index + 1]),
        );
      }
    }
  });
  ...
  it("keeps sibling signature stable across additional mixed metadata variants", () => {
    ...
    for (const variant of variants) {
      const source = readFixture(variant);
      const formatted = formatXmlConservative(source);
      expect(extractSiblingBlockSignature(formatted)).toEqual(
        extractSiblingBlockSignature(source),
      );
      expect(extractElementOrder(formatted)).toEqual(
        extractElementOrder(source),
      );
  ...
  it("keeps child-sequence signature stable across metadata families", () => {
    ...
    for (const variant of variants) {
      const source = readFixture(variant);
      const formatted = formatXmlConservative(source);
      expect(extractChildSequenceSignature(formatted)).toEqual(
        extractChildSequenceSignature(source),
      );
      expect(extractSiblingBlockSignature(formatted)).toEqual(
        extractSiblingBlockSignature(source),
      );
  ...
  it("keeps start-tag attribute signatures stable across broad corpus-like metadata variants", () => {
    ...
    for (const variant of variants) {
      const source = readFixture(variant);
      const formatted = formatXmlConservative(source);
      expect(extractStartTagAttributeSignature(formatted)).toEqual(
        extractStartTagAttributeSignature(source),
      );
      expect(extractStartTagAttributeValueSignature(formatted)).toEqual(
        extractStartTagAttributeValueSignature(source),
      );
      expect(extractChildSequenceSignature(formatted)).toEqual(
        extractChildSequenceSignature(source),
      );
...
function readFixture(relativePath: string): string {
  return readFileSync(
    path.resolve(import.meta.dirname, "../../tests/metadata", relativePath),
    "utf8",
  );
}
